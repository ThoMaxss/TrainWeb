...
export class NotificationService {
  ...
  connect() {
    if (this.isMockMode()) {
      console.info('[NotificationService] Mock mode enabled, skipping WebSocket connect');
      return;
    }
    ...
    try {
      ...
      this.ws.onerror = (error) => {
        console.warn('[NotificationService] WebSocket error (suppressed in demo):', error);
      };
...
export function getNotificationService(): NotificationService {
  if (!notificationService) {
    const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:5232/ws';
