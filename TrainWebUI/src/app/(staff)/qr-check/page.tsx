...
import { getAllBookingsByUser } from "@/lib/api/booking"
...
export default function QRCheckPage() {
  ...
  const ensureBookingsLoaded = async (): Promise<ApiBookingDto[]> => {
    ...
    setError(null)
    try {
      // BE exposes user-scoped bookings; if staff needs all, consider BE admin endpoint.
      const data = await getAllBookingsByUser(user.id!)
