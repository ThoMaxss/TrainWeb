...
export default function AdminStaffPage() {
  ...
  const staffCount = users.filter((u) => (u.role || u.userRole) === UserRole.Staff).length;
  const adminCount = users.filter((u) => (u.role || u.userRole) === UserRole.Admin).length;
  const passengerCount = users.filter((u) => (u.role || u.userRole) === UserRole.Passenger).length;
  ...
  const handleOpenEditDialog = (user: UserDto) => {
    ...
    setFormData({
      ...
      role: user.role || user.userRole || UserRole.Staff,
    });
    setIsAddEditDialogOpen(true);
  };
    ...
    <div className="min-h-screen bg-background">
      <AdminPageHeader
        ...
        stats={[
          ...
          { icon: Activity, label: "Quản trị viên", value: adminCount },
        ]}
        actions={
