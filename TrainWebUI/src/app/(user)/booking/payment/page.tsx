...
import { generateTicketsForBooking } from "@/lib/api/ticket";
...
export default function PaymentPage() {
  ...
  const markBookingPaidOnce = async (bookingId: string) => {
    const key = `paid:${bookingId}`;
    if (sessionStorage.getItem(key) === 'true') return;
    await successBooking(bookingId);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Ensure booking status update is called exactly once per booking
      ...
      // Generate tickets for each booking using stored passenger names
      try {
        const pendingRaw = sessionStorage.getItem('pendingBooking');
        const pending = pendingRaw ? JSON.parse(pendingRaw) : null;
        const passengerNames: string[] = Array.isArray(pending?.passengers)
          ? pending.passengers.map((p: any) => (p.fullName || p.name || 'Hành khách'))
          : [];

        // If we don't have per-booking names, still generate one ticket per booking
        const namesForBookings = bookings.map((_, idx) => passengerNames[idx] || `Passenger ${idx + 1}`);

        await Promise.all(
          bookings.map(async (b, idx) => {
            await generateTicketsForBooking(b.id!, [namesForBookings[idx]]);
          })
        );
      } catch (genErr) {
        console.warn('⚠️ Ticket generation failed or incomplete:', genErr);
      }
