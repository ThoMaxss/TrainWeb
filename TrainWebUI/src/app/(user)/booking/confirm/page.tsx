...
export default function BookingConfirmPage() {
	...
	async function handleContinue() {
		...
		try {
			...
			// 2) Tạo Booking theo BE (POST /api/Booking)
			const bookings = await Promise.all(selectedSeats.map(async (_seat, idx) => {
				...
				try {
					...
					const errorMsg = bookingErr?.response?.data?.message || bookingErr?.message || "Không thể tạo đơn đặt vé"
